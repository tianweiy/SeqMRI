...
class MICCAI2020Env(ActiveMRIEnv):
    ...
    def step(
        ...
        obs, reward, done, meta, gt = super().step(action)
        if self.obs_includes_padding:
            obs["mask"][:, self.START_PADDING : self.END_PADDING] = 1
        return obs, reward, done, meta, gt
...
class SyntheticEnv(ActiveMRIEnv):
    def __init__(
        self,
        ...
        if self.args.noise_type == "none":
            json_name = args.dataset_name
        else:
            json_name = args.dataset_name+"-noise"
        self._setup(f"configs/{json_name}.json", self._create_dataset)
...
class FashionMNISTEnv(SyntheticEnv):
    """Convenient class to access FashionMNIST data with RL Methods.
    Loads the configuration from ``configs/fashion-mnist.json``.
    Looks for datasets named "fashion_mnist_{train/val/test}" under the ``data_location`` dir.
    If "test" is not found, it uses "val" folder for test mode.
    Args:
        config_path(str): The path to the JSON configuration file.
        budget(optional(int)): The length of an acquisition episode. Defaults to ``None``,
    ...
    def __init__(
        ...
        super().__init__(args, num_parallel_episodes, budget, seed, image_shape)

    def _create_dataset(self) -> DataInitFnReturnType:
        root_path = pathlib.Path(self._data_location)
        '''
        train_data = FashionMNISTData(self.args, root_path, custom_split='train')
        val_data = FashionMNISTData(self.args, root_path, custom_split='val')
        test_data = FashionMNISTData(self.args, root_path, custom_split='test')
        '''
        train_val_data = FashionMNISTData(
            self.args,
            self._data_location,
            ActiveMRIEnv._void_transform,
        ...
        val_data_size = int(len(train_val_data)/4)
        train_data_size = len(train_val_data) - val_data_size
        train_data, val_data = torch.utils.data.random_split(train_val_data, [train_data_size, val_data_size])

        return train_data, val_data, test_data


class DicomKneeEnv(SyntheticEnv):
    def __init__(
        self,
        args: argparse.Namespace,
        num_parallel_episodes: int = 1,
    ...
    def _create_dataset(self) -> DataInitFnReturnType:
        ...
        print(f'test slices: {len(test_data)}')

        return train_data, val_data, test_data

    
class RealKneeEnv(SyntheticEnv):
    def __init__(
        self,
        args: argparse.Namespace,
        num_parallel_episodes: int = 1,
